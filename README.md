...
[![GitHub Pages](https://img.shields.io/badge/website-online-brightgreen.svg)](https://nullpointerxyz.github.io/kong-data/)

🚀 **在线访问地址**: [https://nullpointerxyz.github.io/kong-data/](https://nullpointerxyz.github.io/kong-data/)
...
16. **🔐 JWT 解析**：支持本地无网络解析 JWT Token（Base64 URLDecode），清晰显示 Header、Payload，自动校验过期时间，确保隐私安全。
17. **🔗 URL 深度解析**：一键展开 URL 上的 Query 参数为清晰易读的表格清单，并支持直接修改参数列表后重新拼接组合为新链接。
18. **🤖 UUID 批量生成**：快速生成标准格式的 UUID v4，能够自定义是否保留横线 (Hyphen) 并支持大写或小写输出。
...
# 方法 2：手动执行
docker build -t kong-data:latest .
docker run -d -p 80:80 --name kong-data kong-data:latest
```
